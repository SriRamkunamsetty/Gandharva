...
const NotesPanel = ({ notes, isAnalyzing, onExportMidi, onExportPdf }: NotesPanelProps) => {
  return (
    <div className="space-y-4">
      ...
      <div className="glass rounded-xl p-4">
        <h3 className="text-sm font-display text-foreground mb-3 flex items-center gap-2">
          ...
          Detected Notes
          {notes.length > 0 && (
            <span className="text-[10px] text-muted-foreground ml-auto">{notes.length} total</span>
          )}
        </h3>
        {notes.length === 0 ? (
