...
// Floating music particles (reduced count for performance)
  ...
  return (
    <div className="absolute inset-0 overflow-hidden pointer-events-none">
      {Array.from({ length: 10 }).map((_, i) => (
        <motion.span
          key={i}
          className="absolute text-primary/20 text-lg select-none"
          initial={{
            x: `${Math.random() * 100}%`,
            y: "110%",
            rotate: 0,
...
// Equalizer bars with staggered animation delays
const EqualizerBars = () => (
